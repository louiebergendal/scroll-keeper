<template>
	...
	>
		...
		>
			<label
				:for="trait.key"
				class="flex padding-left-tiny padding-bottom-nano padding-right-tiny -v-start"
				:class="{
					'font-contrast-lowest': isFontContrastLowest,
				}"
			>
				<input
					type="radio"
					:checked="traitIsSelected(trait.key) || containsKey(trait.key, tempCharacterSheetProp.traits)"
					:value="trait.key"
					:id="trait.key"
...
export default {
	...
	setup(props) {
		...
		const tempCharacterSheet = ref(props.tempCharacterSheetProp);
	...
	watch: {
		...
		tempCharacterSheetProp: {
			handler(newVal) {
				this.tempCharacterSheet = newVal;
			},
			immediate: true,
		},
	},
	methods: {
		...
		traitIsSelected(traitKey) {
			...
			return (traitKey === this.selectedChoiceKey || containsKey());
		},
	},
};
</script>
