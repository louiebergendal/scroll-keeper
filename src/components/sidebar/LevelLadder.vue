...
export default {
	...
	methods: {
		...
		setChildSelectionData(data) {
			...
			if (getAttributeLongName(data.selectedChoiceKey)) {
				this.currentlySelectedLevelType = 'attribute'
			}
			if (getTraitNiceName(data.selectedChoiceKey)) {
				this.currentlySelectedLevelType = 'trait'
			}

			this.setPluppStatus(
				this.currentlySelectedLevelType,
				data.isInvalid,
